    ...
    async onTurn(context: TurnContext) {
        if (context.activity.type === "message") {
            if(context.activity.text.toLowerCase() === "hello") {
                await context.sendActivity(`Welcome to the Codepunk chatbot. Feel free to ask:
* What's new?
* What's the latest podcast episode?
* What's the latest blog post?
* What's new from Michael?
* What's new from Bill?
                `);
            }
            else if(context.activity.text.match(/from bill|by bill/i)) {
                ...
                await context.sendActivity(MessageFactory.attachment(createThumbnailCard(latest.title, latest.summary, latest.link, latest.image.url)));
            }
                ...
                await context.sendActivity(MessageFactory.attachment(createAudioCard(latest.title, latest.summary, latest.link, latest.enclosure.url)));
            }
